...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    const fetchSubjectResults = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch subject registered: ${response.status}`);
        }
        const data = await response.json();
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-100">
      ...
      {/* Student Details */}
      <div>
        <h1 className="text-center text-2xl text-black font-semibold mb-6">Course Registered</h1>
      </div>
      <div className="flex-grow flex items-center justify-center p-6">
        <div className="container mx-auto p-4">
          ...
          <div className="mt-6">
            <h3 className="text-xl text-black font-bold mb-4">Course Registered</h3>
              ...
              <table className="min-w-full bg-white border">
                <thead>
                  <tr>
                    ...
                    <th className="py-2 text-black px-4 border">Subject Type</th>
                ...
                <tbody>
                  {subjectResults.map((subject, index) => (
                    <tr key={index}>
                      <td className="py-2 text-black px-4 border">{subject.sub_code || 'N/A'}</td>
                      <td className="py-2 text-black px-4 border">{subject.sub_desc || 'N/A'}</td>
                      <td className="py-2 text-black px-4 border">{subject.SUBJECT_TYPE || 'N/A'}</td>
                      <td className="py-2 text-black px-4 border">{subject.crhrs|| 'N/A'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ) : (
              <p className="text-black">No subject registered available.</p>
            )}
          </div>
        </div>
      </div>
